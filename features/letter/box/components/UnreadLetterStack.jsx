...
import useUnreadLetterStack from '../hooks/useUnreadLetterStack';
...
/**
 * 안 읽음 탭의 편지 더미.
 * 처음엔 최대 3장이 모아져 있고, 화면을 내리면 모든 편지가 아래로 펼쳐진다.
 ...
 */
const UnreadLetterStack = ({ letters, bottomInset = 0, onPressLetter, onScroll }) => {
  const { isExpanded, progress, gesture, handleScroll } = useUnreadLetterStack({ onScroll });
  ...
  const stageStyle = useAnimatedStyle(() => ({
    height: interpolate(progress.value, [0, 1], [STAGE_HEIGHT, expandedHeight]),
  }));
  ...
  return (
    <GestureDetector gesture={gesture}>
      <ScrollView
        ...
        // 최상단에서 끌어내리는 동작은 모으기에 쓰므로 바운스를 끈다
        bounces={false}
